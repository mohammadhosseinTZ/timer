...
$(document).ready(function () {
  ...
  function updateTimer(className, name, prevNum) {
    ...
    if (prevNum !== pad(name)[0]) {
      ...
      $(`.mot-timer-wrapper ${className} .timer:nth-of-type(2) span`).text(
        pad(name)[0]
      );
      $(`.mot-timer-wrapper ${className} .timer:nth-of-type(2) span`).addClass(
        "active"
      );
    }
    setTimeout(() => {
      $(`.mot-timer-wrapper ${className} .timer:nth-of-type(1) span`).addClass(
        "active"
      );
    }, 1000);

    $(`.mot-timer-wrapper ${className} .timer:nth-of-type(1) span`).text(
      pad(name)[1]
    );
  }
  ...
  function checkPosition() {
    if (window.matchMedia("(max-width:576px)").matches) {
      $(".list-container").click(function () {
        $(this)
          .parent()
          .find($(".links .wrapper ul li:not(.list-container)"))
          .slideToggle("slow");
      });
    }
  }
